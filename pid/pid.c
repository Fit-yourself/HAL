...
PID_State pid_iterate(PID_Calibration calibration, PID_State state) {
    ...
    state.integral += (error * state.time_delta); //积分
    // calculate derivativec
    double derivative = (error - state.previous_error) / state.time_delta; //微分
    ...
    state.previous_error = error; //更改误差值
